...
const ChatPanel = ({ messages, onSend, loading }) => {
    ...
    return (
        ...
        justify-center mt-0.5 bg-zinc-50">
                                {msg.role === "user" ? (
                                    <UserIcon size={14} className='text-zinc-800' />
                                ) : (
                                    <BotMessageSquareIcon size={14} className="text-zinc-800" />
                                )}
                            </div>
                            <div className="flex-1 min-w-0">
                                <p className="text-s font-medium text-zinc-800 mb-1 uppercase
            tracking-wider">
